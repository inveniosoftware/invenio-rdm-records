...
class TestRDMPermissionPolicy(RecordPermissionPolicy):
    ...
    can_read = [
        IfRestricted(
            "record", then_=[RecordOwners()], else_=[AnyUser(), SystemProcess()]
        )
    ]
    can_read_files = [
        IfRestricted(
            "files", then_=[RecordOwners()], else_=[AnyUser(), SystemProcess()]
        )
    ]
...
def test_permission_policy_generators(
    ...
    assert policy(action="publish").generators[0].needs(record=rest_record) == [
        UserNeed(1)
    ]
