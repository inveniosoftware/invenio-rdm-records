...
def test_datacite47_serializer(running_app, full_record_to_dict):
    """Test serializer to DataCite 4.7 JSON"""
...
def test_datacite47_xml_serializer(running_app, full_record_to_dict):
    """Test serializer to DataCite 4.7 XML"""
    expected_data = (
        ...
        "  <relatedIdentifiers>\n"
        '    <relatedIdentifier relationType="IsCitedBy" resourceTypeGeneral="Dataset" '
        'relatedIdentifierType="DOI">10.1234/foo.bar</relatedIdentifier>\n'
        '    <relatedIdentifier relationType="IsVersionOf" '
        'relatedIdentifierType="DOI">10.1234/pgfpj-at058</relatedIdentifier>\n'
        "  </relatedIdentifiers>\n"
        "  <sizes>\n"
        "    <size>11 pages</size>\n"
        "  </sizes>\n"
    ...
    serializer = DataCite45XMLSerializer()
    serialized_record = serializer.serialize_object(full_record_to_dict)

    assert serialized_record == expected_data


def test_datacite47_alternate_identifiers(running_app, minimal_record):
    ...
    minimal_record["metadata"]["identifiers"] = [
        {"identifier": "10.1234/inveniordmtest.1234", "scheme": "doi"}
    ]

    serializer = DataCite47JSONSerializer()
    serialized_record = serializer.dump_obj(minimal_record)

    assert serialized_record["alternateIdentifiers"] == [
        {
            "alternateIdentifier": "https://self-link.com",
            "alternateIdentifierType": "URL",
        },
        {
            "alternateIdentifier": "10.1234/inveniordmtest.1234",
    ...
    assert serialized_record["doi"] == "10.1234/inveniordm.1234"


def test_datacite47_access_right(running_app, minimal_record, set_app_config_fn_scoped):
...
def test_datacite47_serializer_with_unknown_id_schemes(
    running_app, full_modified_record
):
    """Test if the DataCite 4.7 JSON serializer can handle unknown schemes."""
    # this test is there to ensure that there are no KeyErrors during the
    # lookup of unknown PID schemes during DataCite 4.5 serialization
    # if the behaviour of the datacite serializer is changed, the asserts
    # below should probably be adjusted accordingly

    expected_pid_id = {
        "alternateIdentifier": "unknown-1234",
        "alternateIdentifierType": "unknown-scheme",
    }
    expected_pid_id_2 = {
        "alternateIdentifier": "unknown-1234-a",
        "alternateIdentifierType": "unknown-scheme",
    }
    expected_related_id = {
    ...
    assert serialized_record["doi"] == "10.1234/12345-abcde"

    assert expected_pid_id in serialized_record["alternateIdentifiers"]
    assert expected_pid_id_2 in serialized_record["alternateIdentifiers"]
    assert len(serialized_record["alternateIdentifiers"]) == 4
...
def test_datacite47_xml_serializer_with_unknown_id_schemes(
    running_app, full_modified_record
):
    """Test if the DataCite 4.7 XML serializer can handle unknown schemes."""
    serializer = DataCite45XMLSerializer()
...
def test_datacite47_serializer_with_box(running_app, full_geolocation_box_record):
    """Test if the DataCite 4.7 JSON serializer handles geolocation boxes."""
...
def test_datacite47_serializer_with_polygon(
    running_app, full_geolocation_polygon_record
):
    """Test if the DataCite 4.7 JSON serializer handles geolocation polygons."""
...
def test_datacite47_serializer_updated_date(running_app, full_modified_date_record):
    """Test if the DataCite 4.7 JSON serializer adds system updated date."""
...
def test_datacite47_serializer_empty_record(running_app, empty_record):
    """Test if the DataCite 4.7 JSON serializer handles an empty record."""
        ...
        "relatedItems": [
            {
                ...
                "publicationYear": "2018",
                "titles": [{"title": "Astronomy & Astrophysics"}],
                "volume": "678",
                "issue": "10",
                "firstPage": "A149",
                "publisher": "InvenioRDM",
                "relatedItemIdentifier": {
                    "relatedItemIdentifier": "0004-6361",
