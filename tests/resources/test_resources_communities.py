...
def test_republish_with_mulitple_communities(
    ...
    assert len(record.parent.communities.ids) == 3
    response = client.post(
...
def test_remove_community_restricted_to_curators(
    ...
    monkeypatch.setitem(current_app.config, "RDM_ALLOW_OWNERS_REMOVE_COMMUNITY", False)
    ...
    # Reset config
    monkeypatch.setitem(current_app.config, "RDM_ALLOW_OWNERS_REMOVE_COMMUNITY", True)
