...
def get_style_location(style, locale):
    """Parse style and locale.

    Argument location precedence: kwargs > view_args > query
    """
    _default_style = StringCitationSerializer._default_style
    _default_locale = StringCitationSerializer._default_locale

    csl_args = {
        "style": style if style else _default_style,
        "locale": locale if locale else _default_locale
    }
    try:
...
class StringCitationSerializer(MarshmallowJSONSerializer):
    ...
    def serialize_object(self, record, style, location):
        """Serialize a single record.

        :param record: Record instance.
        """
        return get_citation_string(
            self.dump_one(record), record["id"],
            get_style_location(style, location)
        )

    def serialize_object_list(self, records, style, location):
        ...
        return "\n".join(
            [
                self.serialize_object(rec, style, location)
