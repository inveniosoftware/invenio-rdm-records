    ...
    def proxy_pass(f):
        ...
        def _wrapper(self, *args, **kwargs):
            if self.proxy:
                res = self.proxy()
                if res:
                    return res if isinstance(res, Response) else (res, 200)
...
class CantaloupeProxy(IIIFProxy):
    ...
    def record(self):
        ...
        return self.iiif_service.read_record(g.identity, self.uuid_parts['iiif_record_uuid'])._record

    def file_id(self):
        """UUID of file in local filestore"""
        file_meta = self.file_service().read_file_metadata(identity=g.identity, id_=self.uuid_parts['record_id'],
    ...
    def _can_read_files(self):
        ...
        return self.file_service().check_permission(g.identity, 'read_files', record=self.record())
    ...
    def _make_identifier(self, file_uuid):
        ...
        file_path = os.path.join(self.fs_prefix,
    ...
    def _rewrite_url(self, file_uuid):
        ...
        path = request.path.replace(self.iiif_image_uuid, f"{self.iiif_version}/{self._make_identifier(file_uuid)}")
    ...
    def proxy_request(self):
        ...
        try:
            res = requests.request(request.method, url, stream=True, timeout=self.timeout)
            if not res.ok:
                return self._handle_proxy_error(f"IIIF proxy error", res.status_code)

            payload = (
                self._rewrite_info_json(res)
                if request.endpoint == 'iiif.info'
                else res.iter_content(self.chunk_size)
            )

            return Response(
                payload, status=res.status_code, content_type=res.headers['Content-Type']
        )
