...
class ResourceTypeSchemaV1(BaseSchema):
    ...
    type = fields.Str(
        required=True,
        error_messages=dict(required=_("Type must be specified.")),
    )
    subtype = fields.Str()
...
class DateSchemaV1(BaseSchema):
    ...
    def validate_dates(self, data, **kwargs):
        ...
        start = (
            ...
            arrow.get(data.get("end"), "YYYY-MM-DD").date()
            if data.get("end")
            else None
        )
...
class RelatedIdentifierSchemaV1(BaseSchema):
    ...
    scheme = SanitizedUnicode(
        ...
        validate=validate.OneOf(
            choices=SCHEMES,
            error=_(
                "Invalid related identifier scheme. "
...
class MetadataSchemaV1(BaseSchema):
    ...
    _embargo_date = DateString(
        data_key="embargo_date", attribute="embargo_date"
    )
    _community = fields.Nested(
        CommunitySchemaV1, data_key="community", attribute="community"
    )
...
class RecordSchemaV1(BaseSchema):
    ...
    id = fields.Str(attribute="metadata.recid")
