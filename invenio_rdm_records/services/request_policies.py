...
class PolicyEvaluator:
    ...
    @classmethod
    def _evaluate_allowed(cls, enabled, policy_config, identity):
    ...
    @classmethod
    def _evaluate_policies(cls, enabled, policy_config, identity, record):
...
class RDMRecordDeletionPolicy(PolicyEvaluator):
    ...
    def evaluate(cls, identity, record):
        ...
        return {
            "immediate_deletion": cls._evaluate_policies(
                "RDM_IMMEDIATE_RECORD_DELETION_ENABLED",
                "RDM_IMMEDIATE_RECORD_DELETION_POLICIES",
                identity,
                record,
            ),
            "request_deletion": cls._evaluate_policies(
...
class FileModificationPolicyEvaluator(PolicyEvaluator):
    ...
    def evaluate(cls, identity, record):
        ...
        return {
            "immediate_file_modification": cls._evaluate_policies(
...
class QuotaIncreasePolicyEvaluator(PolicyEvaluator):
    ...
    def evaluate(cls, identity, record):
        ...
        return {
            "immediate_quota_increase": cls._evaluate_policies(
                "RDM_IMMEDIATE_QUOTA_INCREASE_ENABLED",
                "RDM_IMMEDIATE_QUOTA_INCREASE_POLICIES",
                identity,
                record,
